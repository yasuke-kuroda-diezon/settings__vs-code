---
agent: agent
description: 顧客管理タグ(CustomerTag)や入出荷管理タグ(ArrivalShippingTag)、コンタクト管理タグ(ContactTag)の既存実装を参考にして、管理画面に受注管理タグ(ReceiveOrderTag)を新規構築したい。
...
顧客管理タグ(CustomerTag)や入出荷管理タグ(ArrivalShippingTag)、コンタクト管理タグ(ContactTag)の既存実装を参考にして、管理画面に受注管理タグ(ReceiveOrderTag)を新規構築したい。
...
既存の管理タグ機能の構成に合わせる形で、フロントエンド部分(Next.js)の受注管理タグ(ReceiveOrderTag)を実装してください。

・受注管理タグ一覧(新規構築)：regolith-admin/src/app/(authentificated)/(standard)/receive-order/tag/list
・受注管理タグ詳細(新規構築)：regolith-admin/src/app/(authentificated)/(standard)/receive-order/tag/new
・受注管理タグ新規(新規構築)：regolith-admin/src/app/(authentificated)/(standard)/receive-order/tag/edit/[code]
・受注一覧(実装追加対象です)：regolith-admin/src/app/(authentificated)/(standard)/receive-order/(receive-order)/list/page.tsx
・受注詳細(実装追加対象です)：regolith-admin/src/app/(authentificated)/(standard)/receive-order/(receive-order)/edit/[serial]/page.tsx
・受注新規(実装追加対象です)：regolith-admin/src/app/(authentificated)/(standard)/receive-order/(receive-order)/new/page.tsx
...
受注にタグ付けして管理できる、受注管理タグ機能を提供する。
受注管理タグは、タグ種別を持たないことに注意してください。(入出荷管理タグとは、タグ種別部分のみが異なります。)

受注管理タグ一覧：

- （※タグ種別は無い事に注意してください。）
- リファレンス(あり or なし)のチェックボックスで、中間テーブルの有無(ReceiveOrderReceiveOrderTag)を対象にフィルタリング可能。ありを選択した場合は、少なくとも1つ以上の受注に紐づいているタグを表示。なしを選択した場合は、どの受注にも紐づいていないタグを表示。
- タグ名で部分一致検索可能
- 受注管理タグ名、受注管理タグコード、リファレンスを一覧表示
...
- 用途の任意入力
- リファレンスを表示
- タグマージモーダルから、既存の受注管理タグとマージ可能（マージ元とマージ先のタグは単数選択を想定する）

受注一覧：

- 「受注管理タグ」でフィルタリング可能
- 一覧に「受注管理タグ」の登録データを表示

受注詳細・新規：

- （※タグ種別は無い事に注意してください。）
- 「受注管理タグ」で複数の受注管理タグの登録・解除が可能
- 「受注管理タグ」マスタが多い場合は「もっと見る」ボタンからモーダルで選択可能
...
関連ファイルは、文字列"contactTag"で検索してください。

## ユビキタス(コンタクト管理タグ)

コンタクト：Contact
コンタクト管理タグ：ContactTag
（※コンタクト管理タグにタグ種別はありません。）

## パス(コンタクト管理タグ)

コンタクト管理タグ一覧：regolith-admin/src/app/(authentificated)/(standard)/contact/tag/list
コンタクト管理タグ詳細：regolith-admin/src/app/(authentificated)/(standard)/contact/tag/new
コンタクト管理タグ新規：regolith-admin/src/app/(authentificated)/(standard)/contact/tag/edit/[code]

コンタクト一覧：regolith-admin/src/app/(authentificated)/(standard)/contact/(contact)/list/page.tsx
コンタクト詳細：regolith-admin/src/app/(authentificated)/(standard)/contact/(contact)/edit/[serial]/page.tsx
コンタクト新規：regolith-admin/src/app/(authentificated)/(standard)/contact/(contact)/new/page.tsx

## 機能概要(コンタクト管理タグ)

コンタクトにタグ付けして管理できる、コンタクトタグ機能を提供する。
コンタクト管理タグは、タグ種別を持たないことに注意してください。(入出荷管理タグとは、タグ種別部分のみが異なります。)
...
## ファイル一覧(コンタクト管理タグ)(抜け漏れがあるかもしれません)

### API側(NestJS)(コンタクト管理タグ)(抜け漏れがあるかもしれません)

追加 prisma/migrations/20260115084023_create_contact_tag_and_contact_contact_tag/migration.sql
修正 prisma/schema.prisma
修正 src/admin.gql
修正 src/front.gql
追加 src/modules/user/contacts/contact-tags.admin.resolver.ts
追加 src/modules/user/contacts/contact-tags.service.ts
追加 src/modules/user/contacts/contact-tags.type.ts
修正 src/modules/user/contacts/contact.module.ts
修正 src/modules/user/contacts/contacts.service.ts
修正 src/modules/user/contacts/contacts.type.ts
修正 src/modules/user/contacts/dto/contact-pagination.args.ts
追加 src/modules/user/contacts/dto/contact-tag-pagination.args.ts
追加 src/modules/user/contacts/dto/create-contact-tags.dto.ts
修正 src/modules/user/contacts/dto/create-contact.dto.ts
追加 src/modules/user/contacts/model/contact-tags.model.ts
修正 src/modules/user/contacts/model/contacts.model.ts
修正 src/modules/util/code/code.params.ts

### フロントエンド側（Next.js）(コンタクト管理タグ)(抜け漏れがあるかもしれません)

追加 src/app/(authentificated)/(standard)/contact/tag/edit/[code]/page.tsx
追加 src/app/(authentificated)/(standard)/contact/tag/list/page.tsx
追加 src/app/(authentificated)/(standard)/contact/tag/new/page.tsx
修正 src/common/api/modules/contact.ts
追加 src/common/api/modules/contactTag.ts
修正 src/common/api/modules/master.ts
追加 src/common/app/modules/contactTag.ts
修正 src/common/app/path.ts
修正 src/common/gql/contact.ts
追加 src/common/gql/contactTag.ts
修正 src/common/model/content/operationMemo.ts
修正 src/common/model/user/contact.ts
追加 src/common/model/user/contactTag.ts
修正 src/components/container/form/contact/ContactForm.tsx
修正 src/components/container/form/contact/contactSchema.ts
修正 src/components/container/form/contact/relations/ContactInfoForm.tsx
追加 src/components/container/form/contactTag/ContactTagForm.tsx
追加 src/components/container/form/contactTag/contactTagSchema.ts
追加 src/components/container/form/contactTagMerge/ContactTagMergeForm.tsx
追加 src/components/container/form/contactTagMerge/contactTagMergeSchema.ts
修正 src/components/container/layout/AppNavigation.tsx
追加 src/components/container/modal/contact/contactTag/merge/ContactTagMergeModal.tsx
追加 src/components/container/modal/contact/contactTag/merge/ContactTagMergeModalProvider.tsx
追加 src/components/container/modal/contact/contactTag/table/ContactTagListRow.tsx
追加 src/components/container/modal/contact/contactTag/table/ContactTagListTable.tsx
追加 src/components/container/modal/contact/contactTag/table/ContactTagSearchConditionField.tsx
修正 src/components/container/modules/contact/ContactSearchConditionField.tsx
修正 src/components/container/modules/contact/list/ContactListRow.tsx
修正 src/components/container/page/contact/ContactEditPage.tsx
追加 src/components/container/page/contact/tag/ContactTagEditPage.tsx
追加 src/components/container/page/contact/tag/ContactTagListPage.tsx
